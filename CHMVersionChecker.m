...
static NSString *DAYS_BETWEEN_AUTOMATIC_CHECKS_PREF =
    @"VersionChecker:daysBetweenAutomaticChecks";
static NSString *DAYS_BETWEEN_AUTOMATIC_NORMAL_CHECKS_PREF =
    @"VersionChecker:daysBetweenAutomaticNormalChecks";
static NSString *DAYS_BETWEEN_AUTOMATIC_ALERT_CHECKS_PREF =
    @"VersionChecker:daysBetweenAutomaticAlertChecks";
...
- (void)awakeFromNib {
  ...
  _updateAvailableWindow.title =
      [NSString stringWithFormat:_updateAvailableWindow.title, appName];
  _upToDateWindow.title =
      [NSString stringWithFormat:_upToDateWindow.title, appName];
  _cannotCheckWindow.title =
      [NSString stringWithFormat:_cannotCheckWindow.title, appName];

  int preferenceState =
      [[NSUserDefaults standardUserDefaults] boolForKey:AUTOMATIC_CHECK_PREF]
          ? NSOnState
          : NSOffState;
...
- (IBAction)update:(id)sender {
  //    [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:[_macPAD
  //    productPageURL]]];
...
- (IBAction)changePreference:(id)sender {
  ...
  [[NSUserDefaults standardUserDefaults] setBool:(state == NSOnState)
                                          forKey:AUTOMATIC_CHECK_PREF];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:AUTOMATIC_CHECK_PREF
                    object:sender];
}
...
- (void)macPADErrorOccurred:(NSNotification *)notification {
  //    NSLog( @"Error while checking for new version: %@", [[notification
  //    userInfo] objectForKey:MacPADErrorMessage]);
  //
  ...
  //    forKey:FIRST_TIME_PREF];
}

- (void)macPADCheckFinished:(NSNotification *)notification {
  //    if( [[[notification userInfo] objectForKey:MacPADNewVersionAvailable]
  //    boolValue] ) {
  ...
  //
  //    NSString *releaseText = [[_macPAD releaseDate]
  //    descriptionWithCalendarFormat:@"%Y-%m-%d" timeZone:nil locale:nil]; int
  //    releaseAge = -[[_macPAD releaseDate] timeIntervalSinceNow] / 60.0 / 60.0
  //    / 24.0;
  //
  //    [_updateDescriptionTextField setStringValue:[NSString
  //    stringWithFormat:format,
  //        releaseText, [_macPAD newVersion], releaseAge, [_macPAD
  //        currentVersion]]];
  //
...
- (void)updateNewVersionAvailability:(BOOL)isNewVersionAvailable {
  ...
  if (isNewVersionAvailable) {
    [prefs
        setInteger:[prefs
                       integerForKey:DAYS_BETWEEN_AUTOMATIC_ALERT_CHECKS_PREF]
            forKey:DAYS_BETWEEN_AUTOMATIC_CHECKS_PREF];
  } else {
    [prefs
        setInteger:[prefs
                       integerForKey:DAYS_BETWEEN_AUTOMATIC_NORMAL_CHECKS_PREF]
            forKey:DAYS_BETWEEN_AUTOMATIC_CHECKS_PREF];
  }
...
- (BOOL)shouldAutomaticallyCheckForNewVersion {
  ...
  if ([prefs boolForKey:AUTOMATIC_CHECK_PREF]) {
    int daysSinceLastCheck =
        -[[prefs objectForKey:LAST_CHECK_DATE_PREF] timeIntervalSinceNow] /
        (60 * 60 * 24);
    NSLog(@"CHMVersionChecker: %d days since last time", daysSinceLastCheck);

    return (daysSinceLastCheck >=
            [prefs integerForKey:DAYS_BETWEEN_AUTOMATIC_CHECKS_PREF]);
  }
